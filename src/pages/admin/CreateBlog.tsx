// src/pages/admin/CreateBlog.tsx
...
export default function CreateBlogPage() {
  ...
  const [media, setMedia] = useState<File | null>(null); // optional media
  const [preview, setPreview] = useState<string | null>(null);
  const [message, setMessage] = useState<string>("");
  const [loading, setLoading] = useState(false);

  // Handle selecting an image/video for preview (optional)
  ...
  // Common submit logic for draft vs. publish
  const submitBlog = async (isDraft: boolean) => {
    console.log("submitBlog called with isDraft =", isDraft);
    ...
    if (!isDraft) {
      // Ensure slug, title, and content are non-empty
    ...
    const payload: BlogInput = {
      ...
      // (you can add summary / media_cid here if needed)
    };
    ...
    try {
      ...
      // Redirect to the list (ensure this matches your route)
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    console.log("handleSubmit (publish) clicked");
    submitBlog(false); // publish
  };

  const handleSaveDraft = (e: FormEvent) => {
    e.preventDefault();
    console.log("handleSaveDraft clicked");
    submitBlog(true); // save as draft
  };
